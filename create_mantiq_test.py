"""
Ishlatish:
  python create_mantiq_test.py

mantiq_savollar_100ta.md dagi 100 ta savolni shuffle qilib,
10 ta testga bo'ladi — har testda takrorlanmas 10 ta savol.
"""

import os
import re
import sys
import random

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'core.settings')
sys.path.insert(0, BASE_DIR)

import django
django.setup()

from quiz.models import Test, Question, Choice


def parse_questions(md_path):
    with open(md_path, encoding='utf-8') as f:
        content = f.read()

    questions = []
    for block in re.split(r'\n---\n', content):
        q_match = re.search(r'\*\*\d+\.\*\*\s+(.+)', block)
        if not q_match:
            continue
        q_text = q_match.group(1).strip()
        choices = []
        for m in re.finditer(r'-\s+[A-D]\)\s+(.+)', block):
            raw = m.group(1).strip()
            is_correct = '✅' in raw
            choices.append((raw.replace('✅', '').strip(), is_correct))
        if q_text and len(choices) == 4 and any(c[1] for c in choices):
            questions.append({'text': q_text, 'choices': choices})

    return questions


def create_test(questions_data, title):
    test = Test.objects.create(
        title=title,
        description="12 yoshlilar uchun mantiq testi — 10 ta savol",
        duration_minutes=10,
        is_active=True,
    )
    for order, q_data in enumerate(questions_data, start=1):
        q = Question.objects.create(test=test, text=q_data['text'], order=order)
        for text, is_correct in q_data['choices']:
            Choice.objects.create(question=q, text=text, is_correct=is_correct)
    return test


def main():
    md_path = os.path.join(BASE_DIR, 'mantiq_savollar_100ta.md')
    if not os.path.exists(md_path):
        print(f"Xato: {md_path} topilmadi!")
        sys.exit(1)

    all_questions = parse_questions(md_path)
    print(f"Fayldan o'qildi: {len(all_questions)} ta savol")

    if len(all_questions) < 100:
        print("Xato: 100 ta savol kerak!")
        sys.exit(1)

    random.shuffle(all_questions)
    groups = [all_questions[i * 10:(i + 1) * 10] for i in range(10)]

    print("10 ta test yaratilmoqda...\n")
    for i, group in enumerate(groups, start=1):
        title = f"Mantiq testi №{i}"
        test = create_test(group, title)
        print(f"  ✓ {title} — ID={test.id}, URL=/enter/{test.id}/")

    print("\nHammasi tayyor! 10 ta test, har birida 10 ta savol.")


if __name__ == '__main__':
    main()
